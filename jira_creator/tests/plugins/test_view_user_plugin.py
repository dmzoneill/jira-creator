...
class TestViewUserPlugin:
    ...
    def test_register_arguments(self):
        ...
        mock_parser.add_argument.assert_called_once_with(
            "account_id", help="The user's account ID or username"
        )
    ...
    def test_rest_operation_with_account_id(self):
        ...
        plugin = ViewUserPlugin()
        mock_client = Mock()
        mock_client.request.return_value = {"accountId": "12345", "displayName": "John Doe"}

        result = plugin.rest_operation(mock_client, account_id="12345")

        mock_client.request.assert_called_once_with(
            "GET", "/rest/api/2/user?accountId=12345"
        )
    ...
    def test_rest_operation_fallback_to_username(self):
        ...
        mock_client.request.assert_any_call(
            "GET", "/rest/api/2/user?accountId=johndoe"
        )
        mock_client.request.assert_any_call(
            "GET", "/rest/api/2/user?username=johndoe"
        )
        assert result == {"accountId": "12345", "displayName": "John Doe"}
