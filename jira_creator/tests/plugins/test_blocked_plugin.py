...
class TestBlockedPlugin:
    ...
    def test_rest_operation_no_user_specified(self):
        ...
        mock_client.request.return_value = {"name": "current.user", "accountId": "12345"}
    ...
    def test_rest_operation_with_blocked_issues(self, mock_env_get, capsys):
        ...
        with patch.object(plugin, 'rest_operation') as mock_rest:
            ...
            for issue in issues:
                fields = issue["fields"]
                is_blocked = (
                    fields.get("customfield_10001", {}).get("value") == "True"
                )
                if is_blocked:
